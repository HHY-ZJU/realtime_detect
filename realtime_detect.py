...
#加载model
    ...
    videoroot='/mnt/datashare/MIP/Videofilter/20150104001030008.jpg'
    ...
    while(success):
        ...
        label = imgclassify(frame)
        if label == '0':
            loc, labelmap = imgfeature(frame)
            # vis = im.copy()
            im=frame.copy()
    ...
    # videoWriter = cv2.VideoWriter('/mnt/datashare/MIP/Videofilter/saveVideo1.avi',cv2.cv.CV_FOURCC('M','P','4','2'),24,(1920,1080))#最后一个是保存图片的尺寸
    # for each in videos:
